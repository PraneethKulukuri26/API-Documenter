            ...
            {pairs.map((p, i) => (
                <div key={p.id}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <button onClick={() => set(i, 'enabled', !p.enabled)}
                            style={{
                                width: '16px', height: '16px', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                                border: '1.5px solid #FFFFFF', background: p.enabled ? '#FFFFFF' : 'transparent', transition: '150ms ease', cursor: 'pointer'
                            }}>
                    ...
                    {/* Key input */}
                    <input value={p.key} onChange={e => set(i, 'key', e.target.value)} placeholder={keyPlaceholder}
                        style={{
                    ...
                    {/* Value input */}
                    <input value={p.value} onChange={e => set(i, 'value', e.target.value)} placeholder={valuePlaceholder}
                        style={{
                            width: '100%', boxSizing: 'border-box', fontFamily: 'monospace', fontSize: '11px',
                            padding: '6px 10px', borderRadius: '8px',
                            background: '#0A0A0A', border: '1px solid #2A2A2A', color: '#FFFFFF',
