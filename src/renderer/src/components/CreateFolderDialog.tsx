...
import { useCreateFolder, useUpdateFolder, useFolder } from '@/hooks/useFolders'

export function CreateFolderDialog() {
    const { currentProjectId, setShowCreateFolder, editingFolderId, setEditingFolderId } = useAppStore()
    const create = useCreateFolder()
    const update = useUpdateFolder()
    const { data: existing } = useFolder(editingFolderId)
    const isEdit = !!editingFolderId

    const [name, setName] = useState('')
    const [desc, setDesc] = useState('')

    useEffect(() => { if (existing) { setName(existing.name); setDesc(existing.description) } }, [existing])
    useEffect(() => {
        const h = (e: KeyboardEvent) => { if (e.key === 'Escape') close() }
        window.addEventListener('keydown', h); return () => window.removeEventListener('keydown', h)
    }, [])

    const close = () => { setShowCreateFolder(false); setEditingFolderId(null) }
    const submit = async () => {
        if (!name.trim() || !currentProjectId) return
        isEdit && editingFolderId
            ? await update.mutateAsync({ id: editingFolderId, name: name.trim(), description: desc.trim() })
            : await create.mutateAsync({ projectId: currentProjectId, name: name.trim(), description: desc.trim() })
        close()
    }
            ...
            <div onClick={e => e.stopPropagation()}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                        ...
                        <p style={{ fontSize: '13px', color: '#9CA3AF', marginTop: '4px', marginBottom: 0 }}>Group endpoints for organization</p>
                    </div>
                ...
                <div style={{ marginTop: '24px' }}>
                    ...
                    <div>
                        ...
                        <input value={name} onChange={e => setName(e.target.value)} placeholder="Auth" autoFocus
                    ...
                    <div style={{ marginTop: '20px' }}>
                        ...
                        <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Authentication endpoints"
                            style={{
                                width: '100%', boxSizing: 'border-box', height: '90px', padding: '12px', fontSize: '13px',
                                background: '#0F0F0F', border: '1px solid #2A2A2A', borderRadius: '10px',
                                color: '#FFFFFF', outline: 'none', resize: 'none', transition: '150ms ease',
